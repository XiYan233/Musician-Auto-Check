...
def receiveCheck():
    ...
    check = requests.get(api + "/musician/cloudbean/obtain?id=" + userMissionId + "&period=" + period,
                         cookies=login_cookie)
    check1 = requests.get(api + "/musician/cloudbean/obtain?id=" + userMissionId1 + "&period=" + period1,
                          cookies=login_cookie)
    check2 = requests.get(api + "/musician/cloudbean/obtain?id=" + userMissionId2 + "&period=" + period2,
                          cookies=login_cookie)
    check3 = requests.get(api + "/musician/cloudbean/obtain?id=" + userMissionId3 + "&period=" + period3,
                          cookies=login_cookie)
    if check.status_code == 200:
        print("领取登录音乐人中心云豆成功！")
...
def push():
    if pushswitch == "true":
        ...
        push = requests.post(url='https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=' + robot_key, headers=headers,
                             json=data)
